...
fn hash_public_keys<C: BlsSignatureImpl>(
    ...
    sorted_keys.sort_by(|a, b| {
        a.0.to_bytes().as_ref().cmp(b.0.to_bytes().as_ref())
    });
    ...
    for i in 0..sorted_keys.len() {
        ...
        if repr_bytes.len() >= 32 {
            ...
            return Err(BlsError::InvalidInputs("Field representation too small".to_string()));
        }
...
pub fn aggregate_secure<C: BlsSignatureImpl>(
    ...
    let mut paired: Vec<(PublicKey<C>, <C as Pairing>::Signature)> = public_keys.iter()
        .copied()
        .zip(signatures.iter().copied())
        .collect();
    paired.sort_by(|a, b| {
        a.0.0.to_bytes().as_ref().cmp(b.0.0.to_bytes().as_ref())
    });
...
pub fn verify_secure_message_augmentation<C: BlsSignatureImpl, B: AsRef<[u8]>>(
    ...
    verify_secure_with_dst::<C, B>(public_keys, signature, msg, <C as BlsSignatureMessageAugmentation>::DST)
}
...
mod tests {
    ...
    fn test_rogue_key_attack_protection() {
        ...
        assert!(fake_agg_sig.verify_secure(&[pk1, pk2, pk_rogue], msg).is_err());
    }
    ...
    fn test_empty_keys() {
        ...
        assert!(verify_secure_basic::<Bls12381G1Impl, _>(&empty_keys, identity_sig, b"msg").is_ok());
        ...
        if let Signature::Basic(raw_sig) = sig {
            assert!(verify_secure_basic::<Bls12381G1Impl, _>(&empty_keys, raw_sig, b"msg").is_err());
        }
    }
    ...
    fn test_multi_signature_aggregation() {
        ...
        let signatures: Vec<_> = signers.iter().map(|(sk, _)| {
            match sk.sign(SignatureSchemes::Basic, msg).unwrap() {
                Signature::Basic(s) => s,
                _ => panic!("Expected Basic signature"),
            }
        }).collect();
        ...
        assert!(wrapped_sig.verify_secure(&public_keys, b"wrong message").is_err());
    }
}