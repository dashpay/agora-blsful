...
fn test_cpp_rust_two_signers() {
    ...
    assert!(final_sig.verify_secure(&public_keys, &MESSAGE_HELLO).is_ok());
}
...
fn test_normal_aggregation_fails_secure_verify() {
    ...
    assert!(normal_sig.verify_secure(&[pk1, pk2], &MESSAGE_HELLO).is_err());
}
...
fn test_rust_to_cpp_compatibility() {
    ...
    assert!(secure_sig.verify_secure(&[pk1, pk2], &MESSAGE_HELLO).is_ok());
}
