...
fn test_g2_legacy_serialization_roundtrip() {
    ...
    let sig_modern = Signature::<Bls12381G2Impl>::from_bytes_with_mode(&modern_bytes, SignatureSchemes::Basic, false).unwrap();
    ...
    let sig_legacy = Signature::<Bls12381G2Impl>::from_bytes_with_mode(&legacy_bytes, SignatureSchemes::Basic, true).unwrap();
    // Compare actual signature values, not projective coordinates
...
fn test_cross_format_deserialization_fails() {
    ...
    assert!(result.is_err(), "Modern bytes should not deserialize with legacy mode");
    ...
    for i in 0..100 {
        ...
        if (test_legacy[0] & 0x80) == 0 {
            ...
            assert!(result.is_err(), "Legacy Y=0 bytes should not deserialize with modern mode");
            found_y0 = true;
            break;
        }
    }
...
fn test_format_detection() {
    ...
    assert!(matches!(detected_modern, SerializationFormat::Unknown | SerializationFormat::Modern));
...
fn test_verify_secure_legacy_compatibility() {
    ...
    assert!(sig_legacy.verify_secure_with_mode(&[pk1, pk2, pk3], msg, true).is_ok());
    
    // Verify that modern aggregated signature verifies with modern mode
    assert!(sig_modern.verify_secure_with_mode(&[pk1, pk2, pk3], msg, false).is_ok());
    
    // Cross-mode verification should fail
    assert!(sig_legacy.verify_secure_with_mode(&[pk1, pk2, pk3], msg, false).is_err());
    assert!(sig_modern.verify_secure_with_mode(&[pk1, pk2, pk3], msg, true).is_err());
}
...
fn test_legacy_coefficient_generation_differs() {
    ...
    let agg_legacy = aggregate_secure_with_mode::<Bls12381G2Impl>(
        &[pk1, pk2],
        &raw_sigs,
        true,
    ).unwrap();
    
    let agg_modern = aggregate_secure_with_mode::<Bls12381G2Impl>(
        &[pk1, pk2],
        &raw_sigs,
        false,
    ).unwrap();
...
fn test_legacy_bit_patterns() {
    ...
    assert!(modern_bytes[0] & 0x80 != 0, "Modern format should have compression bit set");
