...
fn test_secure_aggregation_coefficients() {
    ...
    println!(
        "\nVerifySecure with secure aggregation (correct order): {:?}",
        verify1
    );

    // Test secure aggregation with reversed order
    let secure_agg2 =
        AggregateSignature::from_signatures_secure(&[sig2, sig1], &[pk2, pk1]).unwrap();
    ...
    println!(
        "VerifySecure with secure aggregation (reversed order): {:?}",
        verify2
    );
    ...
    assert!(
        verify2.is_ok(),
        "Should succeed with secure aggregation in reversed order"
    );
    ...
    assert!(
        normal_verify.is_err(),
        "Should fail with normal aggregation"
    );
}
